...
function Get_Link_Host($link, $host, $pass, $download_file, $ip)
{
    ...
    if (count(explode('account has reach bandwidth limit', $content)) > 1) {
        $entry1 .= '[b][color=purple]Server '.$sever.'out bandwidth[/color][br][color=green]Quá trình xử lý link của bạn bị lỗi. vui lòng post lại một lần nữa sau 30 giây ![/color][br][color=red]Failed to process your links. Please try again in 30 seconds.[/color][/b]';
        return $entry1;
    }
    // lay link
...
function Get_Link_Host_Multi($listlink, $host, $pass, $download_file, $ip, $download_file2)
{
    ...
    foreach ($listlink as $export) {
        ...
        $filename1 = substr($filename, 10, 20) ;
